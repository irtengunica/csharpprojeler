...
int main() {
	...
	cvNamedWindow("Kameradan Kaçamazsýnýz", 1);
	...
	while (true) {
		...
		brezelDetector.detectMultiScale(frame, brezels, 1.1, 3,
			0 | CV_HAAR_SCALE_IMAGE, Size(200, 200));
		...
		printf("buldummm! x2=%d y2=%d  \n\r", hedefx, hedefy);
		if (hedefx >= 350)
		{
			int hedefxyed = hedefx - 350;
		...
		if (konumx >= 2200 || konumx <= 800) {
			konumx = 1500; konumy = 1500;
		}

		if (konumy >= 1800 || konumy <= 1000)  {
			konumx = 1500; konumy = 1500;
		}
		...
		imshow("Kameradan Kaçamazsýnýz!!!", frame);
